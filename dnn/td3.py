...
class Critic:
  ...
  def value_optimization_loss(Y):
    def loss(y_true, y_pred):
      error0 = Y[0] - y_true
      error1 = Y[1] - y_true
      return K.mean(K.square(error0)) + K.mean(K.square(error1))
    return loss
  ...
  def build_model(self):
    ...
    model.compile(optimizer=optimizer, loss=Critic.value_optimization_loss(model.outputs))
    return model
  ...
  def train_on_batch(self, states, actions, Q_targets):
    self.model.train_on_batch(x=[states, actions], y=[Q_targets, Q_targets])
...
class TD3():
  ...
  def learn(self, experiences):
    ...
    noise = self.noise.sample()      
    actions_next = np.clip(actions_next*self.offset_scale + noise, -1., 1.)
    lo = self.bounds[:, 0]
    hi = self.bounds[:, 1]
    scale = (hi - lo)*0.5
    shift = scale + lo
    actions_next = (actions_next + shift)*scale

    Q1, Q2 = self.critic.target_predict_on_batch(next_states, actions_next)
    Q_targets_next = np.minimum(Q1, Q2)

    Q_targets = rewards + self.gamma * Q_targets_next * (1 - dones)
    self.critic.train_on_batch(states, actions, Q_targets)

    if self.steps % self.policy_update == 0:
      learning = 0
...
if __name__ == '__main__':
  ...
  o.run_model(max_epochs=1, n_solved=0, r_solved=90, verbose=0, render=True, training=False)
